...
- ✅ Aucune base de données, `localStorage` ou `sessionStorage` utilisé.
...
- Score éphémère volontairement : il disparaît si l’utilisateur quitte la page.
...
- Persistre le meilleur score localement si le produit évolue vers une vraie gamification — mais cela nécessitera une décision explicite sur `localStorage`.
